...
# Helper function to process PDF documents concurrently
import pypdfium2
from io import BytesIO

# Helper function to process PDF documents concurrently using pypdfium2 without saving them to a temp file
async def get_pdf_content(pdf_docs: List[UploadFile]) -> str:
    async def extract_text_from_pdf(pdf: UploadFile) -> str:
        text_content = ""
        try:
            # Read the uploaded PDF file into memory (in-memory file-like object)
            pdf_data = await pdf.read()
            pdf_stream = BytesIO(pdf_data)  # Create an in-memory byte stream

            # Open the PDF using pypdfium2 from the in-memory stream
            pdf_document = pypdfium2.PdfDocument(pdf_stream)

            # Extract text from each page
            for page_num in range(len(pdf_document)):
                page = pdf_document[page_num]
                text_content += page.get_textpage().get_text_bounded()
    ...
    # Process all PDFs concurrently and gather text
    text_contents = await asyncio.gather(*[extract_text_from_pdf(pdf) for pdf in pdf_docs])
...
async def create_and_save_vectorstore(session, text_chunks: List[Dict[str, str]]) -> str:
    ...
    for data in text_chunks:
        embedding_batch.append(data["text_chunk"])

    try:
        embeddings_batch = await asyncio.to_thread(embeddings.embed_documents, embedding_batch) # Running embedding generation in a separate thread
        
        for i, data in enumerate(text_chunks):
            embedding = np.array(embeddings_batch[i], dtype="float32")
